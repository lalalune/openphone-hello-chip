...
A candidate PDK must therefore expose **at least two** drive-strength variants
per output pad, and must offer either a slew-controlled variant or a
programmable slew bit so that the LED outputs can be slowed down for EMC on the
board. Per-pin slew control is not required.
...
| ----- | ------ | ---------------------------------------------------------- |
| HBM   | >= 2 kV | Standard JEDEC JS-001 Class 2; covered by all open shuttle PDKs. |
...
| -------------- | ------- | ------------------------ | ------------------------ | --------------------------------------------------------------------- |
| SkyWater Sky130| 130 nm  | `sky130_fd_io`           | Efabless / Tiny Tapeout  | Mature open IO cells, documented HBM 2 kV; 3.3 V IO, 1.8 V core fit.  |
...
| Open-source flow supported (OpenLane/OpenROAD) | high | yes | yes | partial | Drives the rest of the signoff chain. |
...
A PDK is **eligible** only if every `hard` axis is `yes` and every `high` axis
is `yes` or has a credible mitigation. The first eligible PDK to also clear a
