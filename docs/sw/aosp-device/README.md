...
```text
sw/aosp-device/device/openagent/openagent_ai_soc
```

External AOSP checkout path:

```text
device/openagent/openagent_ai_soc
```
...
manifests/openagent-ai-soc-local.xml
AndroidProducts.mk
openagent_ai_soc.mk
BoardConfig.mk
device.mk
...
`hwcomposer.openagent_ai_soc` HAL source or reviewed prebuilts that fail closed
...
`hwcomposer.openagent_ai_soc` HAL binaries that fail closed when their backing
...
```sh
/path/to/OpenAgent-AI-SoC/sw/aosp-device/import-aosp-device.sh /path/to/aosp
cd /path/to/aosp
...
```sh
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp lunch
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp vendorimage
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp checkvintf
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp sepolicy-build
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp selinux-neverallow
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp cts-vts-plan
/path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp cuttlefish-smoke
AOSP_QEMU_SMOKE_COMMAND='/exact/qemu-system-riscv64 smoke command' \
  /path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp qemu-smoke
AOSP_RENODE_SMOKE_COMMAND='/exact/renode smoke command' \
  /path/to/OpenAgent-AI-SoC/sw/aosp-device/capture-aosp-evidence.sh /path/to/aosp renode-smoke
...
The Cuttlefish boot capture defaults to `AOSP_PRODUCT=openagent_ai_soc-userdebug`
...
|---|---|
| `docs/evidence/android/openagent_ai_soc_lunch.log` | `build/envsetup.sh`, `device/openagent/openagent_ai_soc/AndroidProducts.mk`, `TARGET_PRODUCT=openagent_ai_soc` |
| `docs/evidence/android/openagent_ai_soc_vendorimage.log` | `out/target/product/openagent_ai_soc/vendor.img`, `out/target/product/openagent_ai_soc/installed-files-vendor.txt`, `out/target/product/openagent_ai_soc/vendor/etc/vintf/manifest/openagent_e1.xml` |
| `docs/evidence/android/openagent_ai_soc_checkvintf.log` | `checkvintf` output against `out/target/product/openagent_ai_soc/vendor` and `openagent_e1.xml` |
| `docs/evidence/android/openagent_ai_soc_sepolicy_build.log` | `m vendor_sepolicy.cil selinux_policy`, `e1_npu_device`, and `hal_e1_npu_default` |
| `docs/evidence/android/openagent_ai_soc_selinux_neverallow.log` | `m sepolicy_neverallows` and `e1_npu` neverallow coverage |
| `docs/evidence/android/openagent_ai_soc_cts_vts_plan.log` | CTS/VTS build or list-module output, selected smoke scope, exclusions, and result directory path |
| `docs/evidence/android/cuttlefish_riscv64_smoke.log` | Cuttlefish launch or `cvd start`, `adb` smoke checks, `ro.product.cpu.abi=riscv64`, and `openagent_ai_soc` |
...
| `docs/evidence/android/renode_e1_soc_smoke.log` | Renode monitor/UART smoke transcript against the OpenAgent model and Android-capable handoff when available |
...
`manifests/openagent-ai-soc-local.xml` is a local-manifest starting point for
...
If `lunch openagent_ai_soc-userdebug` is not visible, add the product makefile
...
| Local manifest seed | `manifests/openagent-ai-soc-local.xml` | Records the intended repo workspace path for mirrored integrations. |
| Product list | `device/openagent/openagent_ai_soc/AndroidProducts.mk` | Exposes `openagent_ai_soc-userdebug` to `lunch`. |
| Lunch product | `device/openagent/openagent_ai_soc/openagent_ai_soc.mk` | Inherits generic AOSP product glue and the OpenAgent device makefile. |
| Board config | `device/openagent/openagent_ai_soc/BoardConfig.mk` | Declares riscv64 target and vendor policy directories. |
| Product makefile | `device/openagent/openagent_ai_soc/device.mk` | Copies init, fstab, and the empty VINTF scaffold; HAL packages stay disabled until evidence exists. |
| Init | `device/openagent/openagent_ai_soc/init.openagent.rc` | Creates the e1 device namespace and starts the NPU HAL only when enabled. |
| Fstab | `device/openagent/openagent_ai_soc/fstab.openagent` | Documents first vendor/data mount contract for simulator integration. |
| VINTF manifest | `device/openagent/openagent_ai_soc/manifest.xml` | Reserves graphics-composer and e1_npu names in comments only. |
| SELinux contexts | `device/openagent/openagent_ai_soc/sepolicy/file_contexts` | Labels HAL binaries and `/dev/e1-npu`. |
| SELinux types | `device/openagent/openagent_ai_soc/sepolicy/e1_npu.te` | Defines the fail-closed NPU device and HAL domains. |
| Kernel fragment | `device/openagent/openagent_ai_soc/kernel/openagent_ai_soc.fragment` | Records Android kernel config needed by the scaffold. |
| DTS scaffold | `device/openagent/openagent_ai_soc/dts/openagent-e1-android.dts` | Mirrors the central platform contract for Android-facing nodes. |
| HAL runtime skeleton | `device/openagent/openagent_ai_soc/hal/e1_npu_runtime.cc` | Host-buildable fail-closed probe for absent `/dev/e1-npu`; always reports `nnapi_acceleration=false` in local checks. |
| HAL probe CLI | `device/openagent/openagent_ai_soc/hal/e1_npu_probe_main.cc` | CLI used by `sw/check_bsp_scaffolds.py aosp` to verify absent-device behavior without fake device evidence. |
| HAL plan | `device/openagent/openagent_ai_soc/hal/README.md` | Defines fail-closed behavior required before package claims are enabled. |
...
| `hwcomposer.openagent_ai_soc` | framebuffer/display node | Expose a simple framebuffer path only; no GLES or Vulkan claim. |
