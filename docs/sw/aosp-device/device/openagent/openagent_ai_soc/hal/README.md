...
`hwcomposer.openagent_ai_soc`, or active VINTF HAL entries to the product until
...
c++ -std=c++17 -Wall -Wextra -Werror \
  sw/aosp-device/device/openagent/openagent_ai_soc/hal/e1_npu_runtime.cc \
  sw/aosp-device/device/openagent/openagent_ai_soc/hal/e1_npu_probe_main.cc \
  -I sw/aosp-device/device/openagent/openagent_ai_soc/hal \
  -o /tmp/e1_npu_probe
...
- `hwcomposer.openagent_ai_soc`: bind only to a proven framebuffer or DRM node.
...
`hwcomposer.openagent_ai_soc` remain blocked until external AOSP evidence exists.
