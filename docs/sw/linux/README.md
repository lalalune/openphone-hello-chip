...
sw/linux/configs/openagent_e1.fragment
sw/linux/dts/openagent-e1.dts
sw/linux/dts/Makefile
sw/platform/generated/e1-platform.dtsi
sw/linux/Documentation/devicetree/bindings/openagent/openagent,e1-*.yaml
...
linux: scaffold audit
  ...
  dependency blocker: external Linux kernel checkout plus integration of drivers/misc/openagent-e1
  status: clear
...
`drivers/misc/openagent-e1`, `sw/linux/dts/openagent-e1.dts` copied into
`arch/riscv/boot/dts/openagent`, the OpenAgent DT bindings copied under
`Documentation/devicetree/bindings/openagent`, and board-owned
Kconfig/Makefile fragments that select `CONFIG_OPENAGENT_E1_NPU` and
...
capture path or match its `openagent-evidence` markers, command markers, and
...
mkdir -p /path/to/linux/drivers/misc/openagent-e1
rsync -a sw/linux/drivers/e1/ /path/to/linux/drivers/misc/openagent-e1/
cp sw/linux/dts/openagent-e1.dts sw/linux/dts/Makefile \
  /path/to/linux/arch/riscv/boot/dts/openagent/
cp sw/platform/generated/e1-platform.dtsi \
  /path/to/linux/arch/riscv/boot/dts/openagent/
cp sw/linux/Documentation/devicetree/bindings/openagent/*.yaml \
  /path/to/linux/Documentation/devicetree/bindings/openagent/
cp sw/platform/generated/e1_platform_contract.h /path/to/linux/drivers/misc/openagent-e1/e1_platform_contract.h
cp sw/linux/configs/openagent_e1.fragment /path/to/linux/kernel/configs/openagent_e1.config
```
...
obj-$(CONFIG_OPENAGENT_E1_BSP) += openagent-e1/
```

```text
# drivers/misc/Kconfig
source "drivers/misc/openagent-e1/Kconfig"
```
...
`docs/evidence/linux/openagent-linux-boot-artifacts.json`.

Set these paths before capture:

```sh
export OPENAGENT_LINUX_TREE=/path/to/linux
export OPENAGENT_BUILDROOT_TREE=/path/to/buildroot
export OPENAGENT_OPENSBI_TREE=/path/to/opensbi
```

Exact command plan for a Linux host or CI worker:

```sh
sw/linux/scripts/import-linux-bsp.sh "$OPENAGENT_LINUX_TREE"
sw/buildroot/scripts/import-buildroot-external.sh "$OPENAGENT_BUILDROOT_TREE"

sw/buildroot/scripts/capture-buildroot-evidence.sh "$OPENAGENT_BUILDROOT_TREE" defconfig
make -C "$OPENAGENT_BUILDROOT_TREE" BR2_EXTERNAL="$PWD/sw/buildroot"
sw/buildroot/scripts/capture-buildroot-evidence.sh "$OPENAGENT_BUILDROOT_TREE" image-manifest

cd "$OPENAGENT_LINUX_TREE"
make ARCH=riscv openagent_e1.config olddefconfig
cd -
sw/linux/scripts/capture-linux-bsp-evidence.sh "$OPENAGENT_LINUX_TREE" kernel-build
sw/linux/scripts/capture-linux-bsp-evidence.sh "$OPENAGENT_LINUX_TREE" dtb-check
...
OPENAGENT_OPENSBI_HANDOFF_CMD='<exact boot command>' \
  docs/sw/opensbi/capture-opensbi-evidence.sh "$OPENAGENT_OPENSBI_TREE" handoff

E1_SMOKE_CMD='<target command for /usr/bin/e1-mmio-smoke>' \
  sw/linux/scripts/capture-linux-bsp-evidence.sh "$OPENAGENT_LINUX_TREE" smoke
...
The `openagent,e1-npu` driver registers `/dev/e1-npu`. `read(2)` returns
...
GEMM workload. It prints `openagent-evidence` markers and a PASS line when the
