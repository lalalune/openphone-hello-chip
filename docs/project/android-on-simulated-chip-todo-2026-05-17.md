...
Scope: everything needed to run Android against the OpenPhone simulated-chip
...
- OpenPhone CPU/AP simulator evidence: generated Chipyard/Rocket AP boot path.
- Current hello-chip RTL evidence: debug/MMIO prototype only, not Android boot.

## Current claim boundary

Android is not proven running on the current hello-chip RTL. The central
platform contract still has `hello_chip.has_cpu=false`, so the checked-in RTL is
...
| DONE | Keep Android simulator reports from claiming hello-chip proof. | `check_android_sim_boot.py` requires reference-only claim boundaries and rejects pass reports unless `scripts/check_software_bsp.py aosp` evidence status clears. |
...
| DONE | Fix default AOSP product mismatch for repo-local product files. | Capture and simulator scripts default to `openphone_ai_soc-userdebug`, which is exported by `AndroidProducts.mk`. |
...
| DONE | Ensure Buildroot kernel fragment enables the parent OpenPhone BSP symbol. | Added `CONFIG_OPENPHONE_HELLO_BSP=m` alongside NPU/DMA module selections. |
...
|---|---|---|---|
| BLOCKER | Generate or import the selected CPU/AP simulator path. | The current hello-chip contract has no CPU. Android needs an AP with reset, DRAM, timer, interrupts, UART, and MMU-capable CPU. | Generated Chipyard/Rocket manifest with DTS, memmap, Verilog/FIRRTL, simulator binary, hashes, and tool versions. |
...
| BLOCKER | Define the AP hardware ABI in the central platform contract. | The platform JSON only describes the debug/MMIO hello chip and not the AP boot ABI. | Contract entries for reset vector, DRAM, UART, CLINT/ACLINT, PLIC/IMSIC, boot ROM/firmware handoff, and generated AP source of truth. |
...
| BLOCKER | Boot Linux on the selected AP simulator. | Android userspace depends on a booting kernel, initrd/rootfs, console, timer, interrupt, and memory path. | `openphone_hello_linux_boot.log`, trap/timer/IRQ log, ISA/cache/MMU log, AP benchmark/sanity log, and CPU/AP evidence manifest hashes. |
...
| BLOCKER | Produce an Android smoke transcript tied to the AP simulator. | Cuttlefish and qemu-virt are reference-only. They do not prove OpenPhone AP ABI. | A transcript from the generated AP simulator showing Android init/userspace progress and explicit claim boundary. |
...
|---|---|---|
| HIGH | Add active Android HAL build rules only after binaries/source exist. | Add reviewed source or prebuilts for `hello_npu.default` and any `hwcomposer.openphone_ai_soc` path, then enable packages and VINTF entries with fail-closed behavior. |
...
| HIGH | Patch external Linux integration into Kconfig/Makefile. | The import helper copies BSP files, but the external kernel must include the OpenPhone driver directory and DT bindings. |
...
| HIGH | Wire `hello_soc_top`/CLINT tests into the standard sim ladder. | Current default cocotb ladder does not cover all boot-relevant top-level behavior. |
