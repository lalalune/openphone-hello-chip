...
Scope: everything needed to run Android against the OpenAgent simulated-chip
...
- OpenAgent CPU/AP simulator evidence: generated Chipyard/Rocket AP boot path.
...
| DONE | Fix default AOSP product mismatch for repo-local product files. | Capture and simulator scripts default to `openagent_ai_soc-userdebug`, which is exported by `AndroidProducts.mk`. |
...
| DONE | Ensure Buildroot kernel fragment enables the parent OpenAgent BSP symbol. | Added `CONFIG_OPENAGENT_E1_BSP=m` alongside NPU/DMA module selections. |
...
| BLOCKER | Boot Linux on the selected AP simulator. | Android userspace depends on a booting kernel, initrd/rootfs, console, timer, interrupt, and memory path. | `openagent_e1_linux_boot.log`, trap/timer/IRQ log, ISA/cache/MMU log, AP benchmark/sanity log, and CPU/AP evidence manifest hashes. |
...
| BLOCKER | Produce an Android smoke transcript tied to the AP simulator. | Cuttlefish and qemu-virt are reference-only. They do not prove OpenAgent AP ABI. | A transcript from the generated AP simulator showing Android init/userspace progress and explicit claim boundary. |
...
|---|---|---|
| HIGH | Add active Android HAL build rules only after binaries/source exist. | Add reviewed source or prebuilts for `e1_npu.default` and any `hwcomposer.openagent_ai_soc` path, then enable packages and VINTF entries with fail-closed behavior. |
...
| HIGH | Patch external Linux integration into Kconfig/Makefile. | The import helper copies BSP files, but the external kernel must include the OpenAgent driver directory and DT bindings. |
