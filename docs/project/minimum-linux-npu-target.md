...
|---|---|---|---|
| Chipyard Linux smoke | `scripts/run_chipyard_openagent_linux_smoke.sh` | `make chipyard-generated-ap-boot` | Attempts generated AP OpenSBI/Linux smoke. |
...
| Linux DTS | `sw/linux/dts/openagent-e1.dts` | `make linux-bsp-check` | Names CPU, timer, PLIC, UART, NPU, DMA, and display nodes for the scaffold. |
| Linux config fragment | `sw/linux/configs/openagent_e1.fragment` | `make linux-bsp-check` | Names kernel symbols needed by the external Linux target. |
...
| Provide the OpenAgent Rocket generator source. | present | The OpenAgent Rocket Scala config exists. | `make chipyard-generator-check` |
...
| Compile and validate the OpenAgent DTB in an external Linux tree. | evidence-required | The real DTB log must include the e1 NPU compatible. | `sw/linux/scripts/capture-linux-bsp-evidence.sh` |
...
| Kernel config enables OpenAgent NPU and DMA drivers. | present | Linux fragment includes OpenAgent driver symbols. | `make linux-bsp-check` |
...
| Kernel, DTB, OpenSBI, serial boot, and smoke evidence manifest exists. | present | `docs/evidence/linux/openagent-linux-boot-artifacts.json` defines required transcript markers. | `make linux-boot-artifacts-check` |
...
2. Capture kernel build and DTB validation with
   ...
   and `dtb-check`.
3. On a Linux host with Chipyard/FireMarshal, generate OpenAgent Rocket
