# Renode Tier-2 Linux boot on the openphone-hello CPU variant

This recipe proves that **our platform contract**
(`sw/platform/hello_platform_contract.json`, section `hello_chip_cpu_variant`)
...
| Path                                                  | Purpose |
|-------------------------------------------------------|---------|
| `sim/renode/openphone_hello.repl`                     | Renode platform model — UART @ 0x10001000, PLIC @ 0x0C000000, CLINT @ 0x02000000, DRAM @ 0x80000000 (256 MiB), DMA/NPU/display stubs at the contract addresses. |
| `sim/renode/openphone_hello_tier2.resc`               | Boot script: loads `fw_payload.elf` (OpenSBI + kernel) and starts the machine, attaching the analyzer to `uart0`. |
| `sim/renode/openphone_hello_tier2.robot`              | Robot test — waits for `openphone tier2: linux booted` and the `/ #` busybox prompt within 60 s. |
| `scripts/sim/run_renode_tier2.sh`                     | Wrapper: runs `renode-test` and tees to `build/sim/renode/tier2.log`. |
| `sw/opensbi/platform/openphone/{platform.c,objects.mk,config.mk,README.md}` | OpenSBI platform glue for our addresses — to be copied into the OpenSBI source tree before building. |
...
- `external/linux/arch/riscv/boot/Image` — riscv64 kernel (rv64gc), with
  the init script printing `openphone tier2: linux booted` before spawning
  busybox sh.
- `build/initramfs/openphone_tier2.cpio.gz` — busybox rootfs.
...
```
OpenSBI v...
Platform Name             : openphone-hello-cpu-variant
Platform Features         : ...
...
`sw/platform/hello_platform_contract.json :: hello_chip_cpu_variant`. The
