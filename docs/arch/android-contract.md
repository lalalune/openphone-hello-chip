...
The first AOSP target should live under `sw/aosp-device/device/openagent/openagent_ai_soc` and boot on QEMU/Renode before RTL simulation is expected to run Android-scale workloads. `make aosp-bsp-check` intentionally fails until that target contains real BoardConfig, init, manifest, SELinux, HAL plumbing tied back to the central contract, and checked-in external build/boot evidence.
