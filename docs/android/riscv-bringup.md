...
adb shell uname -m
adb shell logcat -d -b all > out/openagent-riscv64-logcat.txt
stop_cvd
```
...
adb shell logcat -d -b all > out/openagent-riscv64-home-logcat.txt
stop_cvd
```
...
## OpenAgent AOSP Device Tree Runbook
...
```sh
cd ~/aosp-riscv64
mkdir -p device/openagent
rsync -a /path/to/OpenAgent-AI-SoC/sw/aosp-device/device/openagent/ \
  device/openagent/
source build/envsetup.sh
...
|---|---|---|
| `out/target/product/openagent_ai_soc/vendor.img` | `m vendorimage` | `ls -lh`, build log, VINTF check result |
| `out/target/product/openagent_ai_soc/installed-files-vendor.txt` | AOSP build | HAL/init/fstab entries present |
| `out/target/product/openagent_ai_soc/obj/PACKAGING/vndk_intermediates` | AOSP build | VNDK/Treble packaging log |
| `out/target/product/openagent_ai_soc/obj/ETC/vendor_sepolicy.cil_intermediates/vendor_sepolicy.cil` | AOSP policy build | SELinux policy build transcript |
| `out/openagent-riscv64-logcat.txt` | Cuttlefish run | virtual-device smoke, init, HAL, and SELinux data; not e1_soc boot proof |
...
The `sw/aosp-device/device/openagent/openagent_ai_soc` tree must remain tied to
...
The checked-in `sw/linux/dts/openagent-e1.dts` file is not a complete AP boot
...
|---|---|---|---|
| Graphics composer | `hwcomposer.openagent_ai_soc` and VINTF `android.hardware.graphics.composer@2.4` | `display` MMIO region at `0x10030000` | Stub exposes a framebuffer path only after a Linux display node exists; otherwise service stays disabled or returns unsupported. |
| NPU | `e1_npu.default` and VINTF `vendor.openagent.e1_npu@1.0` | `npu` MMIO region at `0x10020000` and IRQ_NPU | Runtime shim runs fixed-vector smoke only when `/dev/e1-npu` exists; all other ops return unsupported. |
...
2. The OpenAgent device tree and BoardConfig compile far enough to expose
