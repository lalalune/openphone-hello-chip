...
module hello_uart (
    ...
    logic [31:0] baud_div;
    logic [31:0] baud_cnt;
    logic        baud_tick;  // one-cycle pulse per baud period
    ...
    logic [FIFO_ADDR_BITS:0]   tx_count;   // one extra bit to distinguish full/empty
    ...
    // -----------------------------------------------------------------------
    // TX shift register state machine
    // -----------------------------------------------------------------------
    ...
    logic [31:0] rx_baud_cnt;   // separate counter for RX (samples at mid-bit)
    logic        rx_sync0, rx_sync1;  // two-flop synchroniser for rx pin
    ...
    // -----------------------------------------------------------------------
    // Baud tick generator (TX clock)
    // -----------------------------------------------------------------------
    assign baud_tick = uart_en && (baud_div != 32'h0) && (baud_cnt == 32'h0);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (valid && write) begin
                unique case (addr)
                    ADDR_DATA: begin
                        ...
                        // Writes to full TX FIFO are silently dropped (no overrun flag for TX)
                    end
                    ADDR_CONTROL: begin
                        uart_en <= wdata[0];
                        tx_ie   <= wdata[1];
            ...
            // TX FIFO dequeue when TX shift reg loads a new byte
            if (tx_load_byte) begin
                tx_rd_ptr <= tx_rd_ptr + 1'b1;
                tx_count  <= tx_count - 1'b1;
            end

            // RX FIFO enqueue when a byte is received
    ...
    // RX FIFO reset pointer — separate reset for rd_ptr and count
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            rx_wr_ptr <= '0;
            rx_rd_ptr <= '0;
            ...
            // Logic handled in write block above; this block only resets
        end
    end
    ...
    // TX state machine
    // -----------------------------------------------------------------------
    logic tx_load_byte;   // pulses when TX shift reg picks up next byte from FIFO
    ...
    // -----------------------------------------------------------------------
    // RX state machine — samples at mid-bit using its own baud counter
    // -----------------------------------------------------------------------
    logic rx_push_byte;  // pulses when a complete byte has been received
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            unique case (rx_state)
                RX_IDLE: begin
                    rx_baud_cnt <= 32'h0;
                    if (uart_en && !rx_sync1) begin
                        // Falling edge detected — start of start bit
                        // Load half-period so we sample in the middle of the start bit
                        rx_baud_cnt <= (baud_div >> 1);
                        rx_state    <= RX_START;
                    end
                end
                RX_START: begin
                    if (baud_div != 32'h0) begin
                        if (rx_baud_cnt == 32'h0) begin
                            // Sample mid start-bit — confirm it's still low
                            if (!rx_sync1) begin
                ...
                RX_STOP: begin
                    if (baud_div != 32'h0) begin
                        if (rx_baud_cnt == 32'h0) begin
                            // Sample stop bit; byte valid regardless of framing error
                            rx_push_byte <= 1'b1;
                            rx_state     <= RX_IDLE;
                        end else begin
                            rx_baud_cnt <= rx_baud_cnt - 32'h1;
