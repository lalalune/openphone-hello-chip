# Hello Demo — KiCad 7 Project

Evaluation board for the hello chip SoC (`hello_chip_top.sv`).  
Board spec: `docs/board/kicad/hello-demo/fab-notes.md`
...
| File | Purpose |
|---|---|
| `hello-demo.kicad_pro` | KiCad 7 project file — links schematic and PCB, sets design rules, net classes |
| `hello-demo.kicad_sch` | Root schematic (Sheet 1) — hierarchical sheet instantiating the four sub-sheets |
...
| `hello-demo.kicad_pcb` | PCB layout — 100×80mm 4-layer, board outline, stackup, mounting holes, example placements |
...
```sh
kicad hello-demo.kicad_pro
```

This opens the KiCad project manager. From there:

- Click **Schematic Editor** to open `hello-demo.kicad_sch` (root sheet)
- Click **PCB Editor** to open `hello-demo.kicad_pcb`
...
- Sub-sheet files (`soc.kicad_sch`) referenced in the root sheet do not exist yet — the SoC sheet must be created with all `hello_chip_top.sv` ports placed and connected.
...
4. The SoC (`U1 hello_chip_v0`) footprint must be regenerated from the package vendor drawing — the placeholder QFN64 is not release-quality.
...
1. **SoC footprint**: Obtain package vendor drawing for the hello chip QFN64 package. Regenerate `U1` footprint from vendor data. The placeholder `QFN-64-1EP_9x9mm_P0.5mm_EP6.5x6.5mm` is not release-quality.
2. **SoC schematic sheet**: Create `soc.kicad_sch` with all `hello_chip_top.sv` ports (CLK\_IN, RST\_N, DBG\_\*, JTAG\_\*, TEST\_MODE, IRQ\_\*, GPIO\[7:0\]) placed, connected to global net labels, and no-connected where appropriate.
