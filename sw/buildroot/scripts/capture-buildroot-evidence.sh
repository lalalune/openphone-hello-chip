...
record_command() {
	...
	command=$3
	{
		echo "openagent-evidence: target=buildroot artifact=$artifact"
		echo "openagent-evidence: command=$command"
		started=$(timestamp_utc)
		echo "openagent-evidence: started_utc=$started"
		echo "openagent-evidence: buildroot=$buildroot"
		echo "openagent-evidence: br2_external=$external"
...
case "$mode" in
	defconfig)
		record_command \
			openagent_e1_defconfig \
			"$evidence_dir/openagent_e1_defconfig.log" \
			"make BR2_EXTERNAL=$external openagent_e1_defconfig"
		;;
	image-manifest)
		log="$evidence_dir/openagent_e1_image_manifest.txt"
		images="$buildroot/output/images"
		{
			echo "openagent-evidence: target=buildroot artifact=openagent_e1_image_manifest"
			echo "openagent-evidence: command=find output/images -maxdepth 1 -type f -print -exec sha256sum {} ;"
			started=$(timestamp_utc)
			echo "openagent-evidence: started_utc=$started"
			echo "openagent-evidence: buildroot=$buildroot"
			echo "openagent-evidence: br2_external=$external"
			...
			if [ ! -d "$images" ]; then
				{
					...
					echo "openagent-evidence: status=FAIL"
					echo "RESULT=FAIL"
					ended=$(timestamp_utc)
					echo "openagent-evidence: ended_utc=$ended"
					echo "END_UTC=$ended"
				} >> "$log"
				exit 1
			fi
