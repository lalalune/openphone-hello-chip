...
/*
 * OpenAgent e1-DMA platform driver.
 *
 * Binds compatible "openagent,e1-dma" and exports sysfs attributes:
...
static ssize_t bytes_done_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct openagent_e1_dma *dma = dev_get_drvdata(dev);
...
static struct attribute *openagent_e1_dma_attrs[] = {
	&dev_attr_contract.attr,
	&dev_attr_bytes_done.attr,
	&dev_attr_error_count.attr,
	NULL,
};
ATTRIBUTE_GROUPS(openagent_e1_dma);

static int openagent_e1_dma_probe(struct platform_device *pdev)
{
	struct openagent_e1_dma *dma;
	struct resource *res;
	int ret;
	...
	ret = sysfs_create_groups(&pdev->dev.kobj, openagent_e1_dma_groups);
	if (ret)
		return ret;

	dev_info(&pdev->dev,
		 "openagent-e1-dma: phys=0x%llx contract_v%u\n",
		 (u64)res->start, E1_CONTRACT_VERSION);
	return 0;
}

static int openagent_e1_dma_remove(struct platform_device *pdev)
{
	sysfs_remove_groups(&pdev->dev.kobj, openagent_e1_dma_groups);
	return 0;
}

static const struct of_device_id openagent_e1_dma_of_match[] = {
	{ .compatible = "openagent,e1-dma" },
	{ }
};
MODULE_DEVICE_TABLE(of, openagent_e1_dma_of_match);

static struct platform_driver openagent_e1_dma_driver = {
	.probe = openagent_e1_dma_probe,
	.remove = openagent_e1_dma_remove,
	.driver = {
		.name = "openagent-e1-dma",
		.of_match_table = openagent_e1_dma_of_match,
	},
};
module_platform_driver(openagent_e1_dma_driver);
