...
WORK_ORDER = ROOT / "docs/project/security-usb-storage-update-fail-closed-work-order-2026-05-17.yaml"
...
def check_work_order(data: dict, gap_ids: set[str], failures: list[str]) -> None:
    ...
    if not isinstance(boundary, str) or "not implementation evidence" not in boundary or "not implemented" not in boundary:
        failures.append("claim_boundary must state this is not implementation evidence and not implemented")
    ...
    for index, domain in enumerate(domains):
        ...
        negative_text = " ".join(str(item).lower() for item in domain.get("required_negative_evidence", []))
        missing_negative = sorted(term for term in REQUIRED_NEGATIVE_TERMS[domain_id] if term not in negative_text)
        if missing_negative:
            failures.append(f"{domain_id}: required_negative_evidence missing terms: " + ", ".join(missing_negative))
        ...
        if missing_forbidden:
            failures.append(f"{domain_id}: forbidden_claims missing: " + ", ".join(missing_forbidden))
...
def check_cross_links(failures: list[str]) -> None:
    ...
    if feature_data:
        ...
        if "unsigned image rejection" not in " ".join(secure.get("release_evidence", [])).lower():
            failures.append("product feature manifest must name unsigned secure-boot rejection evidence")
