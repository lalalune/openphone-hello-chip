...
def run_mvp_json() -> list[dict[str, str]]:
    ...
    if result.returncode not in (0, 1):
        raise RuntimeError("check_mvp_status.py --json did not produce usable status:\n" + result.stdout)
...
def main() -> int:
    ...
    if extra_rows:
        print("dashboard has MVP rows that are not emitted by check_mvp_status.py: " + ", ".join(extra_rows))
        return 1
