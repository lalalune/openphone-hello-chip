...
def check_artifact_manifest(name: str, errors: list[str]) -> None:
    ...
    if manifest.get("claim_boundary") != "host_checkable_manifest_only_not_boot_evidence":
        errors.append("docs/android/bsp-artifact-manifest.json must keep a non-boot-evidence claim boundary")
    ...
    if sorted(required_repo_evidence) != sorted(expected):
        errors.append(f"{name} artifact manifest evidence list does not match check_software_bsp.py")
    ...
    if not target.get("source_command"):
        errors.append(f"{name} artifact manifest must list the source command for evidence production")
...
def check_log_evidence(path: str, errors: list[str]) -> None:
    ...
    if manifest.get("claim_boundary") != "expected_future_log_markers_only_not_boot_evidence":
        errors.append("docs/android/bsp-log-evidence-manifest.json must keep a non-boot-evidence claim boundary")
    ...
    if not spec.get("producer_command"):
        errors.append(f"docs/android/bsp-log-evidence-manifest.json missing producer_command for {path}")
    if not spec.get("required_metadata"):
        errors.append(f"docs/android/bsp-log-evidence-manifest.json missing required_metadata for {path}")
    if not spec.get("capture_hint"):
        errors.append(f"docs/android/bsp-log-evidence-manifest.json missing capture_hint for {path}")
    if not spec.get("claim_boundary"):
        errors.append(f"docs/android/bsp-log-evidence-manifest.json missing claim_boundary for {path}")
    ...
    if missing_metadata:
        errors.append(f"{path} missing required evidence provenance fields: " + ", ".join(missing_metadata))
    missing_all = [term for term in spec.get("required_all", []) if term not in text]
    if missing_all:
        errors.append(f"{path} missing required log markers: " + ", ".join(missing_all))
    any_terms = spec.get("required_any", [])
    if any_terms and not any(term in text for term in any_terms):
        errors.append(f"{path} missing one of required log markers: " + ", ".join(any_terms))
    forbidden_terms = [term for term in spec.get("forbidden_any", []) if term in text]
    if forbidden_terms:
        errors.append(f"{path} contains forbidden failure/placeholder markers: " + ", ".join(forbidden_terms))
    forbidden_claims = [term for term in spec.get("forbidden_claims", []) if term.lower() in text.lower()]
    if forbidden_claims:
        errors.append(f"{path} contains forbidden broad claim markers: " + ", ".join(forbidden_claims))
...
def check_boot_transcript_schema(errors: list[str]) -> None:
    ...
    if set(environment.get("enum", [])) != expected_envs:
        errors.append("virtual-device smoke schema must enumerate Cuttlefish, QEMU, and Renode evidence environments")
    boundary = properties.get("claim_boundary", {}).get("const")
    if boundary != "virtual_device_smoke_only_not_boot_or_compatibility_evidence":
        errors.append("virtual-device smoke schema must keep a no-boot/no-compatibility claim boundary")
...
def check_aosp_product_glue(errors: list[str]) -> None:
    ...
    if manifest.is_file():
        ...
        if "<hal" in active_text:
            errors.append("AOSP VINTF manifest must not declare active HAL entries until source or prebuilts exist")
    ...
    if "PRODUCT_PACKAGES +=" in device_text and ("hello_npu.default" in device_text or "hwcomposer.openphone_ai_soc" in device_text):
    ...
    for term in forbidden_feature_terms:
        if term in lowered_device_text:
            errors.append(f"AOSP device.mk must not declare Android product feature claim without evidence: {term}")
...
def check_target(name: str) -> tuple[list[str], list[str]]:
    ...
    for path in spec.get("evidence", []):
        check_log_evidence(path, errors)
    if missing_evidence:
        manifest = load_json(LOG_EVIDENCE_MANIFEST, [])
        missing_with_codes = []
        for path in missing_evidence:
            blocker_code = manifest.get("logs", {}).get(path, {}).get("blocker_code", "missing_external_evidence")
...
def target_report(name: str) -> dict:
    ...
    missing_evidence = [path for path in TARGETS[name].get("evidence", []) if not (ROOT / path).is_file()]
    manifest = load_json(LOG_EVIDENCE_MANIFEST, [])
    missing = []
    for path in missing_evidence:
        spec = manifest.get("logs", {}).get(path, {})
...
def main() -> int:
    ...
    if args.json:
        ...
        print(json.dumps({"schema": "openphone.software_bsp_status.v1", "targets": reports}, indent=2, sort_keys=True))
        evidence_required = args.require_evidence and not args.scaffold_only
        return 1 if any(report["errors"] or (evidence_required and report["missing_evidence"]) for report in reports) else 0
