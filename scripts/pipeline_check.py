...
def validate_formal_manifest(root: Path, strict: bool) -> list[str]:
    ...
    if not strict and data.get("release_claim") not in {"strict_requires_sby_and_deep_top", "strict_formal_bmc_evidence"}:
    ...
    for name in ("tflite_cpu", "tflite_hello_npu"):
        ...
        if expected_sha and (root / "benchmarks/models/mobile_smoke.tflite").is_file():
            if result.get("status") == "blocked":
                errors.append(f"{name} is still blocked despite pinned mobile_smoke.tflite artifact")
        elif result.get("status") != "blocked":
            errors.append(f"{name} must stay blocked until a real pinned mobile_smoke.tflite artifact exists")
...
def main() -> int:
    ...
    parser.add_argument("--strict-formal", action="store_true", help="require SBY deep formal evidence")
