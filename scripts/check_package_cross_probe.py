...
def main() -> int:
    ...
    parser.add_argument("--release", action="store_true", help="require board/KiCad cross-probe evidence")
    ...
    if missing_required:
        failures.append("padframe required pins missing from package pinout: " + ", ".join(missing_required))
    ...
    if not kicad_files:
        ...
        if missing_board_nets:
            blockers.append("KiCad files are missing package board nets: " + ", ".join(missing_board_nets))

    if "placeholder" in str(pinout.get("package", "")).lower():
        blockers.append("package pinout still uses a placeholder package name")
    for path in (ROOT / "docs/package/hello-demo-package.md", ROOT / "docs/package/hello-demo-pad-ring.md"):
        text = path.read_text(errors="ignore").lower()
        if "placeholder" in text or "not a foundry-approved" in text:
            blockers.append(f"{path.relative_to(ROOT)} is still a placeholder/draft package artifact")
