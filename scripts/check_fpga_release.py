...
REQUIRED_RELEASE_EVIDENCE = {
    ...
    "nextpnr timing report": ["board/fpga/reports/**/*timing*.rpt", "board/fpga/reports/**/*timing*.txt"],
    "nextpnr route report": ["board/fpga/reports/**/*nextpnr*.log", "board/fpga/reports/**/*route*.rpt"],
    "ecppack transcript": ["board/fpga/reports/**/*ecppack*.log", "board/fpga/reports/**/*pack*.log"],
    "FPGA tool versions": ["board/fpga/reports/**/*tool*version*.txt", "board/fpga/reports/tool_versions.txt"],
}
...
def validate_manifest(blockers: list[str], failures: list[str]) -> None:
    ...
    names = {artifact.get("name") for artifact in artifacts if isinstance(artifact, dict)} if isinstance(artifacts, list) else set()
    for required in {"bitstream", "nextpnr_timing_report", "nextpnr_route_report", "ecppack_transcript", "fpga_tool_versions"}:
...
def main() -> int:
    ...
    parser.add_argument("--release", action="store_true", help="fail when bitstream release evidence is incomplete")
    ...
    if missing_locate:
        blockers.append("FPGA LPF lacks concrete LOCATE COMP assignments for: " + ", ".join(missing_locate))
    if missing_iobuf:
        blockers.append("FPGA LPF lacks concrete IOBUF declarations for: " + ", ".join(missing_iobuf))
