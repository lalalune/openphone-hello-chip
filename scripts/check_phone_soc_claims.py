...
def check_min_blocks(data: dict, errors: list[str]) -> None:
    ...
    for index, block in enumerate(blocks):
        ...
        for path in block.get("current_repo_artifacts", []):
            if isinstance(path, str) and path.endswith((".md", ".yaml", ".json", ".py", ".sv", ".mk")):
...
def check_existing_fail_closed_controls(errors: list[str]) -> None:
    ...
    for target in ("software-bsp-evidence-check", "qemu-check-strict", "renode-check-strict", "pd-signoff-check"):
    ...
    for term in ("placeholder_allowed", "release_blocking", "tflite_hello_npu", "--nnapi_accelerator_name=hello-npu"):
