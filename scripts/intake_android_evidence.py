...
def validate_log(log_path: Path, rel_log: str, spec: dict) -> list[str]:
    ...
    if missing_metadata:
        failures.append(
            f"{rel_log}: missing provenance fields: " + ", ".join(missing_metadata)
        )
    ...
    if required_any and not any(marker in text for marker in required_any):
        failures.append(
            f"{rel_log}: missing one of required markers: " + ", ".join(required_any)
        )
...
def main() -> int:
    ...
    for rel_log in selected_logs(args.target):
        ...
        if not isinstance(spec, dict):
            ...
            continue
        source = candidate_path(args.from_dir.resolve(), rel_log) if args.from_dir else root / rel_log
