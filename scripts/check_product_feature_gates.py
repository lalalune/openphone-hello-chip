...
REQUIRED_SCOPED_TOKENS = {
    ...
    "wifi_bluetooth_gnss_nfc": {
        ...
        "unblock_requires": ("Selected Wi-Fi", "Hardware transcript", "Android Wi-Fi", "RF coexistence"),
    ...
    "sensors_input_haptics": {
        ...
        "unblock_requires": ("Selected IMU", "Hardware transcript", "Android input", "Factory calibration"),
        "android_policy": ("Sensors HAL", "VINTF", "SELinux", "CTS", "CTS Verifier"),
    },
    "battery_pmic_thermal": {
        "commands": ("power_supply", "thermal_zone", "dumpsys battery", "thermalservice", "dumpsys power"),
        "release_evidence": ("Charge", "fuel gauge", "thermal", "brownout", "suspend leakage", "shipping"),
        "unblock_requires": ("Selected battery", "Hardware transcript", "Android Health", "Battery safety"),
...
def check_manifest(data: dict, gap_ids: set[str], failures: list[str]) -> None:
    ...
    if not isinstance(policy, dict):
        ...
        if not isinstance(local_claims, list) or len(local_claims) < 3:
            failures.append("global_policy.local_checks_may_only_claim must list local claim boundaries")
        ...
        if not isinstance(metadata, list):
            ...
            if missing_metadata:
                failures.append("required_evidence_metadata missing: " + ", ".join(missing_metadata))
        ...
        if not isinstance(forbidden_release_claims, list):
            ...
            missing_claims = sorted(REQUIRED_FORBIDDEN_RELEASE_CLAIMS - set(forbidden_release_claims))
            if missing_claims:
                failures.append(
                    "forbidden_release_claims_until_evidence missing: "
                    + ", ".join(missing_claims)
                )
        ...
        if not isinstance(android_evidence, list):
            ...
            if missing_android:
                failures.append(
                    "required_android_release_evidence missing: "
    ...
    for index, domain in enumerate(domains):
        ...
        for field, minimum in (
            ...
            if field == "forbidden_claims":
                for claim in values:
                    if isinstance(claim, str) and not claim.endswith("."):
                        failures.append(f"{domain_id}.forbidden_claims entries must be complete sentences")
            if field == "release_evidence":
                ...
                if not any(token in joined for token in ("transcript", "evidence", "record")):
                    failures.append(f"{domain_id}.release_evidence must name transcripts, evidence, or records")
        ...
        if domain_id in SCOPED_FAIL_CLOSED_DOMAINS:
            ...
            if not isinstance(android_policy, str) or "remain" not in android_policy.lower():
                failures.append(f"{domain_id}.android_declaration_policy must keep declarations fail-closed")
            unblock_requires = domain.get("unblock_requires")
            if not isinstance(unblock_requires, list) or len(unblock_requires) < 4:
                failures.append(f"{domain_id}.unblock_requires must list at least four unblock requirements")
                unblock_requires = []
            ...
            scoped_text = {
                ...
                "release_evidence": " ".join(str(item) for item in domain.get("release_evidence", [])),
    ...
    if missing_domains:
        failures.append("product feature evidence manifest missing domains: " + ", ".join(missing_domains))
    missing_gaps = sorted(REQUIRED_GAP_IDS - seen_gap_ids)
    if missing_gaps:
        failures.append("product feature evidence manifest missing gap links: " + ", ".join(missing_gaps))
