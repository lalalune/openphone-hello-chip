...
def validate_metadata(
    ...
    if isinstance(metadata, dict):
        for key in sorted(k for k in required_keys if CHECKSUM_METADATA_RE.search(k)):
            ...
            if not isinstance(value, str) or not SHA256_RE.fullmatch(value):
                failures.append(f"{field}.metadata.{key}: checksum must be a lowercase sha256 hex digest")
    ...
    if checksum_manifest is not None:
        if not isinstance(checksum_manifest, str) or not checksum_manifest:
            ...
            if path.is_absolute() or ".." in path.parts:
                failures.append(f"{field}: checksum_manifest must be a relative repo path: {checksum_manifest}")
            elif release and not repo_path(checksum_manifest).is_file():
                failures.append(f"{field}: release checksum_manifest is missing: {checksum_manifest}")
...
def check_report_markers(artifact_name: str, artifact: dict, files: list[Path], failures: list[str]) -> None:
    ...
    clean_pattern = re.compile(clean_regex) if isinstance(clean_regex, str) and clean_regex else None
    for path in files:
...
def validate_manifest(path: Path, release: bool) -> list[str]:
    ...
    if not isinstance(status, str) or not status:
        ...
        failures.append(
            f"{manifest_name}: status must be one of "
            + ", ".join(sorted(ALLOWED_MANIFEST_STATUS))
        )
        ...
        if not isinstance(gates, dict):
            ...
            for gate_name, gate in gates.items():
                if not isinstance(gate, dict):
                    failures.append(f"{manifest_name}.release_gates.{gate_name}: gate must be a mapping")
                    continue
                if not isinstance(gate.get("blocked"), bool):
                    failures.append(f"{manifest_name}.release_gates.{gate_name}: blocked must be true or false")
                if release and gate.get("blocked") is True:
                    failures.append(f"{manifest_name}.release_gates.{gate_name}: release gate remains blocked")
    ...
    for group_name, group in groups.items():
        ...
        if group_status not in ALLOWED_STATUS:
            failures.append(f"{manifest_name}.{group_name}: status must be missing, draft, or complete")
        if release and group_status != "complete":
            failures.append(f"{manifest_name}.{group_name}: release requires group status complete, got {group_status}")
            ...
            if isinstance(commands, dict):
                for command_name, command in commands.items():
                    if not isinstance(command, str) or not command:
                        failures.append(f"{manifest_name}.{group_name}.{command_name}: CLI command must be a string")
...
def main() -> int:
    parser = ArgumentParser(description="Validate package, board, SI/PI, current, thermal, and KiCad evidence manifests.")
    parser.add_argument("--manifest", action="append", dest="manifests", help="manifest path to check; may be repeated")
