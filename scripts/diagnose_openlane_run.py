...
def diagnose(run_dir: Path, tail_lines: int) -> tuple[int, str]:
    ...
    earlier_without_state = [step for step in steps[:-1] if not (step / "state_out.json").is_file()]
    blocking_step = last_step if not (last_step / "state_out.json").is_file() else None

    if blocking_step is None and (run_dir / "final").is_dir():
    ...
    if blocking_step is not None:
        ...
        if "klayout-drc" in blocking_step.name:
            lines.append("")
