...
def check_spec_floor(data: dict[str, Any], errors: list[str]) -> None:
    ...
    target = data.get("openagent_target_floor")
    if not isinstance(baseline, dict) or not isinstance(target, dict):
        errors.append("gate must define pinephone_pro_baseline and openagent_target_floor")
        return
...
def require_text_markers(path: Path, markers: list[str], blockers: list[str]) -> None:
    ...
    common_markers = ("RESULT=0", "openagent-evidence: status=PASS")
...
def main() -> int:
    ...
    if data:
        if data.get("schema") != "openagent.aosp_simulator_completion_gate.v1":
