...
def check_selected_manifest(errors: list[str]) -> None:
    ...
    require(manifest.get("schema") == "openphone.cpu_ap_generator_manifest.v1", "unexpected selected manifest schema", errors)
    require(manifest.get("status") == "selected_not_generated", "selected manifest must remain selected_not_generated until artifacts exist", errors)
    ...
    require(selected.get("chipyard_release_commit") == "69eba86", "Chipyard release commit must stay pinned", errors)
    ...
    require(selected.get("harts") == 1, "initial AP integration must be single-hart until boot evidence exists", errors)
    require(selected.get("config_name") == "OpenPhoneRocketConfig", "config name must be OpenPhoneRocketConfig", errors)
    require(policy.get("linux_capable_cpu_claim") is False, "Linux CPU claim must be false without boot evidence", errors)
    require(
    ...
    for path in (
        ...
        require(path in manifest.get("expected_generated_artifacts", []), f"selected manifest lacks generated artifact: {path}", errors)
    ...
    for path in (
        ...
        require(path in manifest.get("required_evidence", []), f"selected manifest lacks evidence artifact: {path}", errors)


def check_generated_import_manifest(errors: list[str]) -> None:
    require(BUILD_MANIFEST.is_file(), f"missing generated import manifest: {BUILD_MANIFEST}", errors)
    if errors:
        return
    ...
    require(manifest.get("schema") == "openphone.cpu_ap_import_manifest.v1", "unexpected generated manifest schema", errors)
    require(chipyard.get("tag") == "1.13.0", "generated manifest uses an unapproved Chipyard tag", errors)
    require(chipyard.get("commit") == "69eba86", "generated manifest uses an unapproved Chipyard commit", errors)
    require(chipyard.get("recursive_submodules_recorded") is True, "generated manifest must record recursive submodules", errors)
    require(bool(chipyard.get("submodules")), "generated manifest must include submodule SHAs", errors)
    require(generation.get("config") == "OpenPhoneRocketConfig", "generated manifest must use OpenPhoneRocketConfig", errors)

    for name in ("verilog", "dts", "simulator"):
        path = artifacts.get(name, "")
        require(bool(path), f"generated manifest lacks artifact path: {name}", errors)
        require(bool(path) and (ROOT / path).exists(), f"generated artifact does not exist: {path}", errors)
    ...
    for name in ("opensbi_boot_log", "linux_boot_log", "trap_timer_irq_log"):
        ...
        require(bool(path) and (ROOT / path).is_file(), f"evidence artifact does not exist: {path}", errors)
...
def main() -> int:
    ...
    if not BUILD_MANIFEST.is_file():
        print(f"STATUS: BLOCKED chipyard.generated_import - missing {BUILD_MANIFEST.relative_to(ROOT)}")
