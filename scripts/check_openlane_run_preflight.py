...
def main() -> int:
    ...
    parser.add_argument("--release", action="store_true", help="require installed pinned image and at least one run directory")
    ...
    if not isinstance(digest_pin, str) or not digest_pin.startswith("sha256:"):
        failures.append("pd/signoff/manifest.yaml runner.openlane_image_digest must be a sha256 digest")
        digest_pin = DEFAULT_OPENLANE_DIGEST

    for config_name in ("pd/openlane/config.json", "pd/openlane/config.sky130.json", "pd/openlane/config.gf180.json"):
    ...
    if not isinstance(run_roots, list) or not run_roots:
        ...
        run_dirs = [
            ...
            for path in (ROOT / run_root).glob("*")
            if path.is_dir()
        ]
        if not run_dirs:
    ...
    if shutil.which("openlane") or shutil.which("flow.tcl"):
        ...
        if manifest_match is False:
            blockers.append(f"OpenLane remote manifest does not contain pinned digest {digest_pin}: {image}")
