...
def check_hello_chip_dram_contract(contract: dict, errors: list[str]) -> None:
    ...
    fail_unless(dram["base"] == "0x80000000", "hello-chip DRAM aperture base must remain 0x80000000", errors)
    fail_unless(dram["size"] == "0x00001000", "hello-chip debug DRAM aperture must remain documented as 4 KiB", errors)
    fail_unless(
...
def check_docs(errors: list[str]) -> None:
    ...
    for token in required_tokens:
        fail_unless(token in joined, f"memory/interconnect docs missing required boundary token: {token}", errors)
    ...
    fail_unless(
        "Real DRAM, PHY timing, refresh, training, ECC, cache coherency, IOMMU/SMMU, and QoS" in memory_subsystem,
        "docs/arch/memory-subsystem.md must explicitly block real memory hierarchy claims",
        errors,
    )
    fail_unless(
        "No release, Android, AI-throughput, display-smoothness, or memory-bandwidth claim" in interconnect,
...
def check_rtl_decode(errors: list[str]) -> None:
    ...
    for name, pattern in required_patterns.items():
        fail_unless(re.search(pattern, rtl), f"interconnect RTL missing {name}", errors)
...
def check_uma_strategy(errors: list[str]) -> None:
    ...
    fail_unless(data.get("schema") == "openphone.uma_coherency_validation_strategy.v1", "UMA strategy schema drifted", errors)
    fail_unless(data.get("status") == "fail_closed_until_evidence", "UMA strategy must fail closed until evidence", errors)
    ...
    for axis_id, gate in REQUIRED_UMA_AXES.items():
        ...
        if not axis:
            continue
        fail_unless(axis.get("release_gate") == gate, f"UMA axis {axis_id} must release through {gate}", errors)
        fail_unless(
        ...
        fail_unless(
            isinstance(axis.get("evidence_gate"), dict) and axis["evidence_gate"].get("blocked") is True,
    ...
    fail_unless(not missing, "UMA strategy missing required artifacts: " + ", ".join(missing), errors)

    for claim_rule in data.get("claim_rules") or []:
        fail_unless("blocked" in claim_rule.lower() or "require" in claim_rule.lower(), f"UMA claim rule is not fail-closed: {claim_rule}", errors)


def check_no_claim_leak(errors: list[str]) -> None:
    combined = "\n".join(read(path) for path in (MEMORY_MAP, MEMORY_SUBSYSTEM, INTERCONNECT, UMA)).lower()
    forbidden_claims = [
