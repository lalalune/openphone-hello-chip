...
def parse(text: str) -> dict[str, Any]:
    ...
    if delegate_m:
        ...
        metrics["unsupported_op_count"] = (
            int(unsupported_m.group(1)) if unsupported_m else fallback
        )
        metrics["cpu_fallback_percent"] = (
            (fallback / total) * 100.0 if total > 0 else 0.0
        )
    elif unsupported_m:
