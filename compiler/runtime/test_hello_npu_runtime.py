...
class CommandCompletingMmio(FakeMmio):
    ...
    def _complete_command(self):
        ...
        if opcode == self.runtime_cls.OP_ADD:
            result = self.regs.get(self.runtime_cls.OP_A, 0) + self.regs.get(self.runtime_cls.OP_B, 0)
            ...
            result = acc + sum(self._s8(a >> (8 * index)) * self._s8(b >> (8 * index)) for index in range(4))
            self.regs[self.runtime_cls.RESULT] = result & 0xFFFF_FFFF
    ...
    def _complete_gemm(self):
        ...
        self.regs[self.runtime_cls.PERF_CYCLES] = self.regs.get(self.runtime_cls.PERF_CYCLES, 0) + macs
